...
def _extract_row_retained(ws_row: Tensor, orig_row: Optional[Tensor],
    ...
    if eW > 0 and tk > 0:
        ev = ws_row[:, :eW].mean(dim=0)
        k = min(tk, eW)
        compact_ev = ev.topk(k).indices.cpu()
    ...
    ret_ids_arr = all_orig[order].numpy()
    ret_compact = all_compact[order].to(dev)
    ret_sc_arr = ws_row[:, ret_compact].cpu().to(torch.float16).numpy()

    all_ids_full = (orig_row.cpu().long().numpy()
                    if orig_row is not None
                    else np.arange(W, dtype=np.int64))
    all_tier_arr = np.full(W, 2, dtype=np.int64)
    all_tier_arr[:eW] = 0
    if q_ids_row is not None and eW > 0:
        q_np = q_ids_row.detach().cpu().long().numpy()
        if q_np.size:
            is_q = np.isin(all_ids_full[:eW], q_np)
            all_tier_arr[:eW][is_q] = 1
...
class OursParityRunner:
    ...
    def run(self) -> Path:
        ...
        if base_gen_tokens.ndim == 1:
            base_gen_tokens = base_gen_tokens[np.newaxis, :]
        ...
        samples_ret_ids: List[np.ndarray] = []
        samples_ret_scores: List[np.ndarray] = []
        samples_win_ids: List[np.ndarray] = []
        samples_win_tier: List[np.ndarray] = []
        ...
        for chunk_start in range(0, num_samples, batch_size):
            ...
            tokens = torch.cat(tok_list, dim=0).to(model.device)
            ...
            all_win_ids    = [[] for _ in range(Bc)]
            all_win_tier   = [[] for _ in range(Bc)]
                    ...
                    for step in range(gen_len):
                        ...
                        for li in range(n_layers):
                            ...
                            if cs.window_scores is not None:
                                layer_wsv.append(cs.window_scores)
                                layer_orig.append(cs.original_window_ids)
                        ...
                        for bi in range(Bc):
                            ...
                            all_win_ids[bi].append(np.stack(step_win_ids, 0))
                            all_win_tier[bi].append(np.stack(step_win_tier, 0))
                            ...
                            all_ret_ids[bi].append(np.stack(p_rid, 0))
                            all_ret_scores[bi].append(np.stack(p_rsc, 0))
            ...
            for bi in range(Bc):
                ...
                samples_win_ids.append(np.stack(pwid, 0))
                samples_win_tier.append(np.stack(pwtier, 0))
                ...
                prsc = [np.pad(x, [(0,0),(0, mH2 - x.shape[-2]),(0, mM2 - x.shape[-1])])
                        ...
                        for x in s_rsc]
                samples_ret_ids.append(np.stack(prid, 0))
                samples_ret_scores.append(np.stack(prsc, 0))
        ...
        retained_window_ids    = np.stack(aligned_ret_ids, 0)
        retained_window_scores = np.stack(aligned_ret_scores, 0)
        all_window_ids         = np.stack(aligned_win_ids, 0)
        all_window_tier        = np.stack(aligned_win_tier, 0)
        ...
        np.savez_compressed(
            ...
            retained_window_scores=retained_window_scores,
            all_window_ids=all_window_ids,
            all_window_tier=all_window_tier,
