...
class RulerRunner:
    ...
    def _predict(
        ...
        #    <|start_header_id|> markers are ordinary text to any other
        #    tokenizer — on Mistral that is a silently mis-formatted prompt that
        #    still produces plausible-looking (and worthless) scores. Same
        #    reasoning as LongBenchRunner._predict; see _load_model_and_tokenizer
        #    for the load-time warning when no template is present.
    ...
    def _setup_windowed_cache(self, input_ids: torch.Tensor, max_gen_len: int):
        ...
        # Window geometry comes from cfg.cache.* — the single source of truth for
        # every runner. The cache/window disagreement this used to warn about can
        # no longer be expressed: utils.config rejects a `window:` block carrying
        # window_size / num_sink_tokens / local_window_size at load time.
