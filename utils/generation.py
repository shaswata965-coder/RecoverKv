...
def unmappable_token_ids(model: Any, tokenizer: Any) -> Optional[List[int]]:
    ...
    returns ``None`` for them and generation is untouched (no logits processor is
    added at all).
    ...
    * ``generate(stop_strings=...)`` **crashes** on one.
    ...
    what the CPU smoke path uses. So this keeps that path usable and removes a
    crash class — not a real result. It cannot change one either: for a trained
    model these ids never win, and for Llama/Mistral the list is empty and no
    logits processor is added at all.
    """
