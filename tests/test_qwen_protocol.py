...
1. **RoPE with a non-unit ``attention_scaling``.** YaRN folds
   ...
   ``factor=4`` that is 1.296x on every int4 key. ``rope_type`` default/llama3
...
class TestRopeRoundTripUnderScaling:
    ...
    def test_without_the_correction_the_error_is_a_squared(self):
        ...
        a²·k_post. At YaRN factor 4 every int4 key would be 29.6% too large.
        """
        a = YARN_FACTOR_4
        rope = _ScaledRope(attention_scaling=a)
        k_post, pos = _key_and_pos()
...
class TestInt4GridUnderBfloat16:
    """The int4 scale/zero grid vs a bf16 KV cache.

    Under fp16 KV the grid cannot overflow (inputs are fp16-bounded, so
    ``scale = (mx-mn)/15 <= 8734``). Under bf16 KV it can, because bf16 carries
    ...
    # The fixture magnitude is tied to _LEVELS: what overflows the fp16 grid is
    # `scale = spread/_LEVELS`, so at int4 the spread must clear 15 * 65504 =
    # 982560 for the pre-fix path to fail the way it did (int2 only needed
    # 3 * 65504). Widened accordingly — the spread this test carried on the int2
    # branch divides down to a perfectly representable scale here, and would
    # have silently stopped testing anything.
    _PAST_FP16 = [[-1.0e6, 0.0, 1.0, 2.0e6]]

    def test_a_key_past_fp16_range_no_longer_corrupts_the_window(self):
        """The actual bug: bf16 holds 2e6, fp16's grid cannot, so it became nan."""
        x = torch.tensor(self._PAST_FP16, dtype=torch.bfloat16)
        ...
        assert back.max() > 1.0e6 and back.min() < -1.0e5
    ...
    def test_fp16_cache_path_is_bit_identical_to_the_original(self):
        ...
        sc = torch.where(mx == mn, torch.ones_like(mx), (mx - mn) / 15.0)
        s16, z16 = sc.to(torch.float16), mn.to(torch.float16)
        q = torch.round((x32 - z16.float()) / s16.float()).clamp(0.0, 15.0)
