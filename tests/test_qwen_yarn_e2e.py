...
``generate()`` with eviction and the int2 tier live, because the failure mode
...
@pytest.mark.parametrize("quant_ratio", [0.0, 0.5], ids=["fp16-only", "two-tier-int2"])
...
def test_dropping_the_correction_inflates_the_key_by_a_squared():
    ...
    factor 4.0, on every int2 key and every Q->fp promotion.
    """
