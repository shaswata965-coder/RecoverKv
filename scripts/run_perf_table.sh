...
#   READ_GATE       the gate ITSELF: on (default) | off. "off" compiles the
...
#                   READ_GATE=off too before quoting the pair. Also --read-gate.
...
# `quant_read_gate` is emitted only when asked for, so the default config is
# byte-identical to what it was and nothing has to re-derive "unset == on".
READ_GATE_YAML=""
case "$READ_GATE" in
  ""|derive|derived) READ_GATE_YAML="" ;;
  off|0|false|no)    READ_GATE_YAML="  quant_read_gate: false" ;;
  on|1|true|yes)     READ_GATE_YAML="  quant_read_gate: true" ;;
  *) echo "ERROR: --read-gate must be on | off (got '$READ_GATE')" >&2; exit 2 ;;
esac
...
cache:
  ...
  quant_gate_ratio: ${GATE_RATIO}${READ_GATE_YAML:+
$READ_GATE_YAML}
...
# ---- environment snapshot, for reproducibility -----------------------------
{
  ...
  echo "read_gate: ${READ_GATE:-derived (on)}"
...
echo "read gate: ratio=$GATE_RATIO  gate=${READ_GATE:-derived (on)}$( [ "$READ_GATE" = "off" ] && echo '   <-- UNGATED ARM: machinery compiled out, read memo restored, scores exact. Quote only beside an accuracy run taken the same way.' )"
