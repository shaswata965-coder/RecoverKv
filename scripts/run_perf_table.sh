...
# Digest-gated decode (DIGEST_GATED_DECODE_PLAN.md). Empty = OFF, which is the
# pre-feature behaviour exactly: no digests allocated, no gating. A float in
# (0,1] admits that fraction of N_q Q-tier windows per decode step.
DIGEST_GATE_FRAC="${DIGEST_GATE_FRAC:-}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --digest-gate-frac)     DIGEST_GATE_FRAC="$2"; shift 2;;
...
# ---- digest gate: emit the knob only when asked for -------------------------
# Left empty the generated config is byte-identical to a pre-feature one, so an
# ungated table is comparable to every table already on record.
if [[ -n "$DIGEST_GATE_FRAC" ]]; then
  DIGEST_CACHE_LINE="  digest_gate_frac: ${DIGEST_GATE_FRAC}"
  DIGEST_CELL_LINE="      digest_gate_frac: ${DIGEST_GATE_FRAC}"
  CELL_SUFFIX="_gate${DIGEST_GATE_FRAC}"
else
  DIGEST_CACHE_LINE=""
  DIGEST_CELL_LINE=""
  CELL_SUFFIX=""
...
perf:
  ...
  configs:
    - name: ours_q${QUANT_RATIO}${CELL_SUFFIX}
...
# ---- environment snapshot, for reproducibility -----------------------------
{
  ...
  echo "digest_gate_frac: ${DIGEST_GATE_FRAC:-off}"
...
echo "=== run_perf_table: q=$QUANT_RATIO ($QUANT_MODE), budget=$CACHE_BUDGET, backend=$BACKEND, digest_gate=${DIGEST_GATE_FRAC:-off} ==="
